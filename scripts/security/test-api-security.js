    ...
    const req = client.request(requestOptions, (res) => {
      ...
      res.on('end', () => {
        ...
        if (verbose) {
          ...
          console.log('Body:', typeof parsedData === 'object' ? JSON.stringify(parsedData, null, 2) : parsedData);
        }
...
async function runTests() {
  ...
  try {
    ...
    logResult('API Connectivity', response.success, 
  ...
  logResult('Protected Routes', noAuthResponse.statusCode === 401, 
  ...
  logResult('Invalid JWT', invalidTokenResponse.statusCode === 401, 
  ...
  logResult('Dev Token Rejected', devTokenResponse.statusCode === 401, 
  ...
  if (devTokenWithHeaderResponse.statusCode === 401) {
    logResult('Dev Token with Header', true, 'Development token with special header properly rejected in production');
  } else {
    logWarning('Dev Token with Header', 
  ...
  if (testTokenResponse.success) {
    logWarning('Test Token', 'Test token endpoint is accessible - this should be disabled in production', 
  ...
  if (corsHeaders['access-control-allow-origin']) {
    ...
    if (allowOrigin === '*') {
      logWarning('CORS', 'CORS allows requests from any origin (*) - consider restricting this in production');
    } else {
  ...
  for (let i = 0; i < 5; i++) {
    rateLimitRequests.push(makeRequest({
      url: `${baseUrl}/api/health`,
      method: 'GET'
    }));
  }
  ...
  const hasRateLimitHeaders = rateLimitResponses.some(response => 
  ...
  if (hasRateLimitHeaders) {
    ...
    logWarning('Rate Limiting', 'No rate limiting headers detected - consider implementing rate limiting');
  }
  ...
  if (missingHeaders.length > 0) {
    logWarning('Security Headers', `Missing recommended security headers: ${missingHeaders.join(', ')}`);
  }
